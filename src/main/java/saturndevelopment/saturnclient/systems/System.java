...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(SaturnClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(SaturnClient.MOD_ID, file.getName());
            ...
            SaturnClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    SaturnClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    SaturnClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            SaturnClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
