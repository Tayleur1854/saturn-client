...
public class Strafe extends SpeedMode {
    ...
    public static Vector2d transformStrafe(double speed) {
        float forward = Math.signum(SaturnClient.mc.player.input.getMovementInput().y);
        float side = Math.signum(SaturnClient.mc.player.input.getMovementInput().x);
        float yaw = SaturnClient.mc.player.getLerpedYaw(SaturnClient.mc.getRenderTickCounter().getTickProgress(true));
