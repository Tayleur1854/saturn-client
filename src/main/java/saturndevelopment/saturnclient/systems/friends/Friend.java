...
package saturndevelopment.saturnclient.systems.friends;

import com.mojang.util.UndashedUuid;
import saturndevelopment.saturnclient.SaturnClient;
import saturndevelopment.saturnclient.utils.misc.ISerializable;
import saturndevelopment.saturnclient.utils.network.FailedHttpResponse;
    ...
    public void updateInfo() {
        ...
        if (id != null) {
            res = Http.get("https://sessionserver.mojang.com/session/minecraft/profile/" + UndashedUuid.toString(id))
                .exceptionHandler(e -> SaturnClient.LOG.error("Error while trying to connect session server for friend '{}'", name))
        ...
        if (res == null || res.statusCode() != 200) {
            res = Http.get("https://api.mojang.com/users/profiles/minecraft/" + name)
                .exceptionHandler(e -> SaturnClient.LOG.error("Error while trying to update info for friend '{}'", name))
                .sendJsonResponse(APIResponse.class);
        }
