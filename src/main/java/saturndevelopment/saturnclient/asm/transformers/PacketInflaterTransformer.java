...
package saturndevelopment.saturnclient.asm.transformers;

import saturndevelopment.saturnclient.asm.AsmTransformer;
import saturndevelopment.saturnclient.asm.Descriptor;
import saturndevelopment.saturnclient.asm.MethodInfo;
        ...
        for (AbstractInsnNode insn : method.instructions) {
            if (insn instanceof TypeInsnNode typeInsn && typeInsn.getOpcode() == Opcodes.NEW && typeInsn.desc.equals("io/netty/handler/codec/DecoderException")) {
                ...
                if (newCount == 2) {
                    ...
                    list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "saturndevelopment/saturnclient/systems/modules/Modules", "get", "()Lmeteordevelopment/meteorclient/systems/modules/Modules;", false));
                    list.add(new LdcInsnNode(Type.getType(AntiPacketKick.class)));
                    list.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "saturndevelopment/saturnclient/systems/modules/Modules", "isActive", "(Ljava/lang/Class;)Z", false));
