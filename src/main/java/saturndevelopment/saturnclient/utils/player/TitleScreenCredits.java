...
import saturndevelopment.saturnclient.utils.render.MeteorToast;
...
public class TitleScreenCredits {
    ...
    private static void init() {
        ...
        credits.sort(Comparator.comparingInt(value -> value.addon == SaturnClient.ADDON ? Integer.MIN_VALUE : -mc.textRenderer.getWidth(value.text)));
        ...
        MeteorExecutor.execute(() -> {
            for (Credit credit : credits) {
                ...
                request.exceptionHandler(e -> SaturnClient.LOG.error("Could not fetch repository information for addon '{}'.", credit.addon.name, e));
                ...
                switch (res.statusCode()) {
                    case Http.UNAUTHORIZED -> {
                        ...
                        SaturnClient.LOG.warn(message);
                        if (System.getenv("meteor.github.authorization") == null) {
                            SaturnClient.LOG.info("Consider setting an authorization " +
                                "token with the 'meteor.github.authorization' environment variable.");
                            SaturnClient.LOG.info("See: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens");
                        }
                    }
                    case Http.FORBIDDEN -> SaturnClient.LOG.warn("Could not fetch updates for addon '{}': Rate-limited by GitHub.", credit.addon.name);
                    case Http.NOT_FOUND -> SaturnClient.LOG.warn("Could not fetch updates for addon '{}': GitHub repository '{}' not found.", credit.addon.name, repo.getOwnerName());
                    case Http.SUCCESS -> {
